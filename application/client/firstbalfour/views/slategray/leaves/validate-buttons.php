  ...
  <div class="icon-label-group">
    <?php if($this->user_access[$this->module_id]['post'] == 1 || $this->user_access[$this->module_id]['hr_health'] == 1):
      $show_or = true;?>
          ...
          <div class="icon-label">
              <a class="icon-16-send-email" href="javascript:void(0);" onclick="sent_to_approver(<?php echo $this->key_field_val?>)">
                  <span>Send To Approver</span>
              </a>            
          </div>
          <div class="icon-label">
              <a class="icon-16-cancel" href="javascript:void(0);" onclick="invalid_request(<?php echo $this->key_field_val?>)">
...
<script>
  ...
  function invalid_request( record_id ){
    $.ajax({
      url: module.get_value('base_url') + module.get_value('module_link') + '/invalid_request',
      type: 'post',
      data: 'record_id='+record_id,
      beforeSend: function(){
        $.blockUI({ message: '<div class="now-loading align-center"><img src="'+module.get_value('base_url')+user.get_value('user_theme')+'/images/loading.gif"><br />Loading, please wait...</div>' });      
