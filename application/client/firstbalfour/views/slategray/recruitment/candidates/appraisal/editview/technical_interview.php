	...
	<div id="form-div">                        
		<div class="" id="fg-143" fg_id="143">
			<div class="col-2-form">
				<div class="form-item odd">
					<div class="form-submit-btn align-left nopadding">
					    ...
					    <div class="icon-label">
					      <a href="javascript:void(0)" class="icon-16-add" onclick="add_technical_interviewer()">                        
				...
				<div id="technical-interviewer-container">
					<?php
						if ($technical_candidate_interviewer && $technical_candidate_interviewer->num_rows() > 0){
							foreach ($technical_candidate_interviewer->result() as $row_interviewer) {
					?>
								<div class="parent_container">
										...
										<label class="label-desc gray" for="final_interview_id">
											...
											<div class="select-input-wrap">
												<?php if (count($interviewer) > 0): ?>
													<select class="final_interviewer_id technical" name="final_interviewer_id[]">
														<option value="">Select...</option>
														<?php foreach ($interviewer as $key => $value) { ?>
															<option value="<?php echo $value['user_id'] ?>" <?php echo ($value['user_id'] == $row_interviewer->user_id ? 'SELECTED="SELECTED"' : '') ?>><?php echo $value['lastname'] ?>,&nbsp;<?php echo $value['firstname'] ?></option>
														<?php } ?>
													</select>
												<?php else: ?>
									...
									<div class="form-item1 even">
										...
										<div class="text-input-wrap"><input type="text" class="input-text technical datetimepicker" name="final_interview_date[]" value="<?= ($row_interviewer->datetime == "0000-00-00 00:00:00" ? "" : $row_interviewer->datetime) ?>"/></div>
									</div>
									...
									<div class="form-item2">
										<div style="padding-top:13px; width:100px;" class="icon-group">
												...
												<a class="icon-button icon-16-send-email icon-send technical" href="javascript:void(0)"></a>
										</div>
									</div>	
				...
				<?php if ($with_sched): ?>
					...
					<div class="form-item even hidden" id="candidate_status_id_container">
						<label class="label-desc gray" for="candidate_status_id">Status:</label>
							<div class="select-input-wrap">
								<?php 
									$this->db->order_by('ordered_by','ASC');
									$status = $this->db->get_where('recruitment_candidate_status',array('can_jump' => 1,'deleted' => 0));

									if ($status && $status->num_rows() > 0): 
								?>
									<select id="candidate_status_id" name="candidate_status_id">
										<option value="">Select...</option>
										<?php foreach ($status->result() as $row) { ?>
											<option value="<?php echo $row->candidate_status_id ?>" <?php echo ($row->candidate_status_id == $candidate_info['candidate_status_id'] ? 'SELECTED="SELECTED"' : '') ?>><?php echo $row->candidate_status ?></option>
										<?php } ?>
									</select>
								<?php endif; ?>
							</div>
