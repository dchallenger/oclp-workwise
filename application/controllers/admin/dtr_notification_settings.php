...
class Dtr_notification_settings extends MY_Controller
{
	...
	function index()
    {
		...
		$data['dtr_notification_settings'] = $this->hdicore->_get_config('dtr_notification_settings');

		$emailto_list = array();
		$emailcc_list = array();

		foreach( $data['dtr_notification_settings']['email_to'] as $email_to_settings ){
		...
		foreach( $data['dtr_notification_settings']['email_cc'] as $email_cc_settings ){
	...
	function edit()
	{
		if($this->user_access[$this->module_id]['edit'] == 1){
			...
			foreach( $data['dtr_notification_settings']['email_cc'] as $email_to_settings ){
	...
	function ajax_save()
	{
		if(IS_AJAX){
			...
			if($this->user_access[$this->module_id]['edit'] == 1){
				...
				$config = array(
					'dtr_notification_settings' => array(
						'email_to' => $email_to_users,
