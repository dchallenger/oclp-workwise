...
class performance_evaluation_percentage_report extends MY_Controller
{
	        ...
	        foreach ($employees as $employee_id) {
	        	...
	        	if($employee_qry && $employee_qry->num_rows() > 0){
					...
					if ($employee_appraisal_bsc_qry && $employee_appraisal_bsc_qry->num_rows() > 0) {
						...
						$employee_com = $employee_appraisal_bsc_row['completed'] == 1 ? '1' : '';
						$employee_opn = '1';
						$employee_onp = $employee_opn == '1' && $employee_com != 1 ? '1' : '';
		            ...
		            $response->rows[$ctr]['cell'][6] = $employee_com;
		            $response->rows[$ctr]['cell'][7] = $employee_onp;
		            $response->rows[$ctr]['cell'][8] = $employee_opn;

		            $employee_com_count += ($employee_com == 1 ? 1 : 0);
					$employee_onp_count += ($employee_opn == 1 && $employee_com != 1 ? 1 : 0);
					$employee_opn_count += ($employee_opn == 1 ? 1 : 0);
					$employee_count++;
		            
		            $ctr++;
				}
				
	            $response->rows[$ctr]['cell'][6] = number_format($employee_com_count / $employee_count * 100, 2).'%';
	            $response->rows[$ctr]['cell'][7] = number_format($employee_onp_count / $employee_count * 100, 2).'%';
	            $response->rows[$ctr]['cell'][8] = number_format($employee_opn_count / $employee_count * 100, 2).'%';
	        }
	    }
	...
	function export() {
        ...
        $objPHPExcel->getProperties()->setTitle("Performance Evaluation Percentage")
                    ->setDescription("Performance Evaluation Percentage");
        ...
        $objPHPExcel->getActiveSheet()->setCellValueExplicit('A1',  'Performance Evaluation Percentage', PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        if($appraisal_planning_period_qry && $appraisal_planning_period_qry->num_rows() > 0){
    			...
    			foreach ($employee_ids as $employee_id) {
    				...
    				if($employee_qry && $employee_qry->num_rows() > 0){
						...
						if($employee_appraisal_bsc_qry && $employee_appraisal_bsc_qry->num_rows() > 0){
							...
							$employee_com = $employee_appraisal_bsc_row['completed'];
							$employee_opn = "1";
						}
						...
						$objPHPExcel->getActiveSheet()->setCellValueExplicit('G'.$line, ($employee_com == 1 ? '1' : ''), PHPExcel_Cell_DataType::TYPE_STRING);
						$objPHPExcel->getActiveSheet()->setCellValueExplicit('H'.$line, ($employee_opn == 1 && $employee_com != 1 ? '1' : ''), PHPExcel_Cell_DataType::TYPE_STRING);
						$objPHPExcel->getActiveSheet()->setCellValueExplicit('I'.$line, ($employee_opn == 1 ? '1' : ''), PHPExcel_Cell_DataType::TYPE_STRING);
			...
			$objPHPExcel->getActiveSheet()->setCellValueExplicit('G'.$line, number_format($employee_com_count / $employee_count * 100, 2).'%', PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->getActiveSheet()->setCellValueExplicit('H'.$line, number_format($employee_onp_count / $employee_count * 100, 2).'%', PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->getActiveSheet()->setCellValueExplicit('I'.$line, number_format($employee_opn_count / $employee_count * 100, 2).'%', PHPExcel_Cell_DataType::TYPE_STRING);
    	}
        ...
        header('Content-Disposition: attachment;filename=Performance_Evaluation_Percentage_'.date('Y-m-d').'.xls');
