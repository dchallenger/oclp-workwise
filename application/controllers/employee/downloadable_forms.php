...
class Downloadable_forms extends MY_Controller
{
	function __construct(){
		...
		if(!$this->is_superadmin && !$this->is_admin && $this->user_access[$this->module_id]['post'] != 1){
			...
			//$this->filter = 'IF(company_recipients <> "",FIND_IN_SET( '.$company_id.', company_recipients ), "")';
        }
	}
	...
	function ajax_save(){
		...
		if( $this->input->post('record_id') && $this->input->post('record_id') == "-1" ){
			$_POST['createdby'] = $this->user->user_id;
		}
		parent::ajax_save();
