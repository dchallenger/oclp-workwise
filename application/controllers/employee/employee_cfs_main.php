...
class Employee_cfs_main extends MY_Controller
{
	...
	function index()
    {
		...
		if($this->user_access[$this->module_id]['post']==0)
		{
			 ...
			 redirect(base_url().'employee/employee_cfs_main/edit','location');
		}
	...
	function edit()
	{
		if($this->user_access[$this->module_id]['post']==0)
		{
			...
			$counted=$this->db->get('employee_cfs_main');
	...
	function ajax_save()
	{
		if($this->input->post('record_id')!='-1')
		{
			...
			if($fromflagCount == 0 && $toflagCount==0)
			{
				...
				$this->db->where('employee_cfs_main_id', $this->input->post('record_id'));
				$this->db->update('employee_cfs_main');
				parent::ajax_save();
			}
			else
			{
		...
		if($this->input->post('is_hr')==1)
		{
			...
			if($flagCount == 0)
			{
				$id=$this->db->get('employee_cfs_main')->num_rows();
				$id=$id+1;
				$_POST['record_id']=$id;
				$this->db->set('date_period_from',date('Y-m-d',strtotime($this->input->post('date_period_from'))));
				$this->db->set('date_period_to',date('Y-m-d',strtotime($this->input->post('date_period_to'))));
				$this->db->set('employee_cfs_main_id',$id);
				$this->db->insert('employee_cfs_main');
				$this->db->set('dummy_date_from',date('Y-m-d',strtotime($this->input->post('date_period_from'))));
				$this->db->set('dummy_date_to',date('Y-m-d',strtotime($this->input->post('date_period_to'))));
				$this->db->set('employee_cfs_main_id',$id);
				$this->db->update('employee_cfs_main');
				...
				$user_info=$this->db->get('user')->row_array();
					...
					{
						for($ctr=1;$ctr<=6;$ctr++){
							$_POST['record_id']=$id['employee_cfs_main_id'];
							...
							$this->db->set('employee_cfs_main_id',$id['employee_cfs_main_id']);
							...
							$this->db->insert('employee_cfs_answer');
						}
					}
					else
					{
	  ...
	  //      	$cfs_id=$this->db->get('employee_cfs_main');
	  //       $flag=0;
	  //       if($cfs_id->num_rows()>0){
	  //           foreach($cfs_id->result_array() as $approved_cfs_id){
	  //           // $this->db->where('employee_cws_id',$approved_cws_id['employee_cws_id']);
	  //           // $date_allowed=$this->db->get('employee_cws_dates')->result_array();
	  //           	$dates_inputted=$this->system->get_affected_dates( '1', $approved_cfs_id['date_period_from'], $approved_cfs_id['date_period_to'] );;
		...
		if (!IS_AJAX) {
			...
			if($this->input->post('is_hr')==1)
			{
				if($this->input->post('record_id') != -1)
				{
					$this->db->where('employee_cfs_main_id', $this->input->post('record_id'));
					$this->db->where('employee_id', $this->input->post('employee_id'));
					$employee=$this->db->get('employee_cfs_answer');
				}
				else
				{
				...
				$this->db->where('date_period_to >=', $curdate);
				$this->db->where('date_period_from <=', $curdate);
				$id=$this->db->get('employee_cfs_main')->row_array();

				$this->db->where('employee_cfs_main_id', $id['employee_cfs_main_id']);
				$this->db->where('employee_id', $this->input->post('employee_id'));
				$employee = $this->db->get('employee_cfs_answer');
	...
	function delete()
	{
		...
		$id=$this->db->get('employee_cfs_main')->result_array();

		foreach($id as $toDelete)
		{
			$this->db->where('employee_cfs_main_id', $toDelete['employee_cfs_main_id']);
			$this->db->delete('employee_cfs_answer');
		}
	...
	function _default_grid_actions( $module_link = "",  $container = "", $record = array() )
	{
		...
		$ir = $this->db->get_where($this->module_table, array($this->key_field => $record['employee_cfs_main_id']));
	...
	function print_record($record_id = 0) {
		...
		$template = $this->template->get_module_template($this->module_id, 'cfs');
		...
		if ($check_record->exist) {
			...
			//dbug($vars);
			$this->db->where('employee_cfs_main_id',$record_id);
			...
			$this->pdf->Output(date($this->config->item('display_datetime_format_compact')).'-cfs-'. $record_id .'.pdf', 'D');
		} else {
	...
	function get_prev_info() {
		if (!IS_AJAX) {
			...
			$this->db->where('employee_cfs_main_id',$this->input->post('record_id'));
			$employee = $this->db->get('employee_cfs_main');
	...
	function get_report() {
		...
		$companywide_count=$this->db->get('employee_cfs_answer');
