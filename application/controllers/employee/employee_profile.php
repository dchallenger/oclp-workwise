...
class Employee_profile extends MY_Controller
{
	function __construct()
    {
		...
		$this->listview_title = 'Employee Profile';
		$this->listview_description = 'This module lists all defined employee profile(s).';
		$this->jqgrid_title = "Employee Profile List";
		$this->detailview_title = 'Employee Profile Info';
		$this->detailview_description = 'This page shows detailed information about a particular employee profile.';
		$this->editview_title = 'Employee Profile Add/Edit';
		$this->editview_description = 'This page allows saving/editing information about employee profile(s).';
    }
	...
	function index()
    {
		...
		$data['content'] = 'employee/employee_profile/listview';
	...
	private function _get_html()
	{
		...
		$template = $this->template->get_module_template($this->module_id, 'employee_profile_with_assessment');
		...
		$exist = file_exists( $results['photo'] );

		if( ( $results['photo'] != "" ) &&  $exist ){
			$results['photo_file'] = '<img src="'.base_url().$results['photo'].'" width="200px" height="200px" />';
		}
		else{
			$results['photo_file'] = '<img src="'.base_url().'themes/default/images/no-photo.jpg" width="200px" height="200px" />';
		}

		$compared_date_tenure = $this->hdicore->compare_date($results['employed_date'], date('Y-m-d'), false );
		$compared_date_age = $this->hdicore->compare_date($results['birth_date'], date('Y-m-d'), false );

		$results['tenure'] = floor( $compared_date_tenure->difference_months / 12 );
		$results['age'] = floor( $compared_date_age->difference_months / 12 );
		...
		foreach($family_result as $family){
			...
			if( $family->relationship == 'Child'){
				$child_count ++;
			}

			if( $family->relationship == 'Spouse' ){
				$results['spouse'] = $family->name;
			}
		...
		if( $child_count != 0 ){
			$results['no_of_children'] = $child_count;
		}
		...
		$employment_result = $this->db->get('employee_employment');

		if( $employment_result->num_rows() > 0 ){

			$employment_result = $employment_result->result();

			foreach($employment_result as $employment){

				$employment_html .= '<tr>
				<td style="font-size:7pt; width:40%; border:1px solid #000; text-align:center;" colspan="2">
					'.$employment->position.'</td>
				<td style="font-size:7pt; width:30%; border:1px solid #000; text-align:center;" colspan="2">
					'.date($this->config->item('display_date_format'),strtotime($employment->from_date)).'</td>
				<td style="font-size:7pt; width:30%; border:1px solid #000; text-align:center;" colspan="2">
					'.date($this->config->item('display_date_format'),strtotime($employment->to_date)).'</td>
			</tr>';
		...
		$results['employment'] = $employment_html;
	...
	function get_html()
	{
		...
		if( ( $results['photo'] != "" ) &&  $exist ){
			$results['photo_file'] = '<img src="'.base_url().$results['photo'].'" width="100px" height="100px" />';
		}
		else{
			$results['photo_file'] = '<img src="'.base_url().'themes/default/images/no-photo.jpg" width="100px" height="100px" />';
		}
		...
		$this->db->select('cup.position AS "current_position"');
		$this->db->select('nup.position AS "new_position"');
		$this->db->select('employee_movement.transfer_effectivity_date AS "from_date"');
		$this->db->where('employee_movement.employee_id',$this->input->post('export_employee_id'));
		$this->db->join('user_position cup','cup.position_id = employee_movement.current_position_id');
		$this->db->join('user_position nup','nup.position_id = employee_movement.new_position_id');
		$this->db->order_by('from_date','DESC');
		$movement_result = $this->db->get('employee_movement');

		//dbug($this->db->last_query());

		if( $movement_result->num_rows() > 0 ){

			$movement_result = $movement_result->result_array();

			dbug($movement_result);

			foreach($movement_result as $key => $val ){

				if( $key > 0 ){

				$movement_html .= '<tr>
				<td style="font-size:7pt; width:40%; border:1px solid #000; text-align:center;" colspan="2">
					'.$movement[$key]['current_position'].'</td>
				<td style="font-size:7pt; width:30%; border:1px solid #000; text-align:center;" colspan="2">
					'.date($this->config->item('display_date_format'),strtotime($movement[$key]['from_date'])).'</td>
				<td style="font-size:7pt; width:30%; border:1px solid #000; text-align:center;" colspan="2">
					'.date($this->config->item('display_date_format'),strtotime($movement[$key-1]['from_date'])).'</td>
				</tr>';
		...
		$results['movement'] = $movement_html;
