...
class Service_award_report extends MY_Controller
{
	function __construct()
    {
		...
		$this->listview_title = 'Service Award Report';
		$this->listview_description = 'This module lists all defined service award report(s).';
		$this->jqgrid_title = "Service Award Report List";
		$this->detailview_title = 'Service Award Report Info';
		$this->detailview_description = 'This page shows detailed information about a particular service award report.';
		$this->editview_title = 'Service Award Report Add/Edit';
		$this->editview_description = 'This page allows saving/editing information about service award report(s).';
    }
	...
	function index()
    {
		...
		$data['content'] = 'employee/service_award_report/listview';
	...
	function listview()
	{
		...
		//build query
		$this->_set_left_join();
		$this->db->select('CONCAT( u.firstname," ",u.lastname ) as "employee"',FALSE); 
		...
		if( $this->input->post('date_asof') && $this->input->post('date_asof') != 'null' ){
			...
			$this->db->where('( FLOOR( ( DATEDIFF("'.date('Y-m-d h:i:s',strtotime($this->input->post('date_asof'))).'", '.$this->db->dbprefix.$this->module_table.'.employed_date ) / 365 ) ) ) >= 5');
			$this->db->where('( FLOOR( DATEDIFF("'.date('Y-m-d h:i:s',strtotime($this->input->post('date_asof'))).'", '.$this->db->dbprefix.$this->module_table.'.employed_date) / 365 ) % 5 ) = 0');

			//$this->db->where('( YEAR( "'.date('Y-m-d h:i:s',strtotime($this->input->post('date_asof'))).'" ) - YEAR( '.$this->db->dbprefix.$this->module_table.'.employed_date ) ) >= 5');
			//$this->db->where('( ( YEAR( "'.date('Y-m-d h:i:s',strtotime($this->input->post('date_asof'))).'" ) - YEAR( '.$this->db->dbprefix.$this->module_table.'.employed_date ) ) % 5 ) = 0 ');

		}
		else{

			$this->db->where('( FLOOR( ( DATEDIFF(NOW(), '.$this->db->dbprefix.$this->module_table.'.employed_date ) / 365 ) ) ) >= 5');
			$this->db->where('( FLOOR( DATEDIFF(NOW(), '.$this->db->dbprefix.$this->module_table.'.employed_date) / 365 ) % 5 ) = 0');

			//$this->db->where('( YEAR( NOW() ) - YEAR( '.$this->db->dbprefix.$this->module_table.'.employed_date ) ) >= 5');
					...
					foreach ($result->result_array() as $row){
						...
						foreach($this->listview_columns as $column => $detail){
							...
							elseif( $column == 0 ){
								...
								if( $this->input->post('date_asof') && $this->input->post('date_asof') != 'null' ){
									$end_of_year = date("Y-m-d",strtotime($this->input->post('date_asof')));
								}
								else{
									$end_of_year = date("Y-m-d");
								}

								$employed_date = new DateTime( date('Y-m-d', strtotime($row['employed_date'])) );
					        	$lastday_of_year = new DateTime( date('Y-m-d', strtotime($end_of_year)) );
								$interval = $lastday_of_year->diff($employed_date);

								$cell[$cell_ctr] = $interval->y;
								$cell_ctr++;
							}else{
	...
	function get_service_award_report_filter(){
		$html = '';
	...
	function export() {	
		...
		if( $this->input->post('date_asof') ){
			...
			$date_asof = date('Y-m-d h:i:s',strtotime($this->input->post('date_asof')));

		}
		else{

			$date_asof = date('Y-m-d h:i:s');
		...
		//$sql_string = ' ( ( YEAR( "'.$date_asof.'" ) - YEAR( e.employed_date ) ) >= 5 ) AND';
		//$sql_string .= ' ( ( ( YEAR( "'.$date_asof.'" ) - YEAR( e.employed_date ) ) % 5 ) = 0 ) ';

		$sql_string = '(( FLOOR( ( DATEDIFF("'.$date_asof.'", e.employed_date ) / 365 ) ) ) >= 5 ) AND';
		$sql_string .= '( FLOOR( DATEDIFF("'.$date_asof.'", e.employed_date) / 365 ) % 5 ) = 0';
		...
		$sql_string .= " ORDER BY e.employed_date ASC ";
	...
	private function _excel_export()
	{
		...
		if($this->input->post('date_asof')){
			$activeSheet->setCellValueExplicit('A2',  'Service Award Report as of '.date($this->config->item('display_date_format'),strtotime($this->input->post('date_asof'))), PHPExcel_Cell_DataType::TYPE_STRING);
		}
		else{
			$activeSheet->setCellValueExplicit('A2',  'Service Award Report as of '.date($this->config->item('display_date_format')), PHPExcel_Cell_DataType::TYPE_STRING);
		}
		...
		foreach ($query->result() as $row) {
			...
			foreach ($fields as $field) {
				...
				if( $field == 'Department' ){
					...
					$employed_date = new DateTime( date('Y-m-d', strtotime($row->{$field})) );
		        	$lastday_of_year = new DateTime( date('Y-m-d', strtotime($end_of_year)) );
					$interval = $lastday_of_year->diff($employed_date);

					$objPHPExcel->getActiveSheet()->setCellValueExplicit($xcoor . $line,  $interval->y, PHPExcel_Cell_DataType::TYPE_STRING);


				}
				else if( $field == 'Date Hired' ){
