...
class Et_blanket extends MY_Controller
{
		...
		if (CLIENT_DIR == "firstbalfour") {
			...
			if( !( $this->is_superadmin || $this->is_admin ) && $this->user_access[$this->module_id]['project_hr'] == 1 ){
				...
				if( count($subordinates) > 0 && $subordinates != false){
					...
					$this->filter .= $dbprefix.'employee_et_blanket.etblanket_id IN ('.implode(',', $blankets).')';
				}else{
					$this->filter .= $dbprefix.'employee_et_blanket.etblanket_id IN (0)';
				}		
	...
	function after_ajax_save()
	{
		if ($this->get_msg_type() == 'success') {
			...
			$data['etblanket_id'] 		 = $this->key_field_val;
			$data['datelate']			 = $record->datelate;
			$data['reason']			 	 = $record->reason;
			$data['documents']			 	 = $record->documents;
			$data['time_difference']     = 1.00;
			...
			$this->db->insert_batch('employee_et', $insert);
		}
		...
		if( IS_AJAX ){
			if($this->user_access[$this->module_id]['delete'] == 1){
				if($this->input->post('record_id')){
					...
					$this->db->where_in($this->key_field, $record_id);
					$this->db->update('employee_et', array('deleted' => 1));
	...
	function filter_employees()
	{
		if (!IS_AJAX) {
			...
			$defaults = array();
			if ($record_id > 0) {
				$this->db->where('etblanket_id', $record_id);
				$this->db->where('deleted', 0);
			...
			$this->db->select('e.employee_id as id, CONCAT(u.firstname, " ", u.lastname) as name, c.city', false);
