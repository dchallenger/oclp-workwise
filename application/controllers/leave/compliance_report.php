...
class Compliance_report extends MY_Controller
{
	function __construct()
    {
		...
		$this->listview_title = 'Compliance Report';
		$this->listview_description = 'This module lists all defined compliance report(s).';
		$this->jqgrid_title = "Compliance Report List";
		$this->detailview_title = 'Compliance Report Info';
		$this->detailview_description = 'This page shows detailed information about a particular compliance report.';
		$this->editview_title = 'Compliance Report Add/Edit';
		$this->editview_description = 'This page allows saving/editing information about compliance report(s).';
    }
	...
	function index()
    {
		...
		$data['scripts'][] ='<link href="'.base_url().'lib/chosen/chosen.css" rel="stylesheet"></link>';
		$data['scripts'][] ='<script type="text/javascript" src="'.base_url().'lib/chosen/chosen.jquery.min.js"></script>';
		$data['content'] = 'leaves/compliance_report/listview';
	...
	{		
		...
		$startcutoff = date('Y-m-d', strtotime('+1 day', strtotime($prevcutof->cutoff)));
		$sql = "SELECT
		...
		FROM (
			...
			'DAILY TIME RECORD PROBLEM' as Particulars
			FROM {$this->db->dbprefix}user u 
			INNER JOIN {$this->db->dbprefix}dtr_daily_summary_lf s ON (s.employee_id = u.employee_id)
		    ...
		    LEFT JOIN {$this->db->dbprefix}employee_dtrp dtrp
		    	ON (dtrp.employee_id = u.employee_id
			        AND dtrp.date BETWEEN '". $prevcutof->date_from ."' AND '". $prevcutof->date_to ."'
			        AND DATE(dtrp.date_approved) BETWEEN '".  $startcutoff . " 00:00:00' AND '" . $p->cutoff . " 23:59:59'
			        AND dtrp.form_status_id = 3) AND dtrp.date = s.date  
			WHERE (s.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "' AND u.deleted = 0)
			AND dtrp.date IS NOT NULL AND s.employee_type = 3
	    	...
	    	LEFT JOIN {$this->db->dbprefix}employee_out eout
		    	ON (eout.employee_id = u.employee_id
			        AND eout.date BETWEEN '". $prevcutof->date_from ."' AND '". $prevcutof->date_to ."'
			        AND DATE(eout.date_approved) BETWEEN '".  $startcutoff . " 00:00:00' AND '" . $p->cutoff . " 23:59:59'
			        AND eout.form_status_id = 3) AND eout.date = s.date
			WHERE (s.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "' AND u.deleted = 0)
			AND eout.date IS NOT NULL AND s.employee_type = 3

			UNION

			SELECT e.id_number, u.lastname, u.firstname, d.department, s.date, time(r.time_in1) as time_in1, time(r.time_out1) as time_out1,
			u.deleted, u.company_id, u.division_id, u.department_id, u.employee_id, e.employed_date, e.resigned_date,e.status_id,e.employee_type,
			'EXCUSED TARDINESS' as Particulars
			FROM {$this->db->dbprefix}user u 
			INNER JOIN {$this->db->dbprefix}dtr_daily_summary_lf s ON (s.employee_id = u.employee_id)
			INNER JOIN {$this->db->dbprefix}user_company_department d ON (u.department_id = d.department_id)
			INNER JOIN {$this->db->dbprefix}employee e ON (u.employee_id = e.employee_id)
	    	...
	    	LEFT JOIN {$this->db->dbprefix}employee_et et
		    	ON (et.employee_id = u.employee_id
			        AND et.datelate BETWEEN '". $prevcutof->date_from ."' AND '". $prevcutof->date_to ."'
			        AND DATE(et.date_approved) BETWEEN '".  $startcutoff . " 00:00:00' AND '" . $p->cutoff . " 23:59:59'
			        AND et.form_status_id = 3) AND et.datelate = s.date AND s.lates > {$this->config->item('deduction_lates')} / 60
			WHERE (s.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "' AND u.deleted = 0)
			AND et.datelate IS NOT NULL AND s.employee_type = 3

			UNION

			SELECT e.id_number, u.lastname, u.firstname, d.department, s.date, time(r.time_in1) as time_in1, time(r.time_out1) as time_out1,
			u.deleted, u.company_id, u.division_id, u.department_id, u.employee_id, e.employed_date, e.resigned_date,e.status_id,e.employee_type,
			'OFFICIAL BUSINESS TRIP' as Particulars
			FROM {$this->db->dbprefix}user u 
			INNER JOIN {$this->db->dbprefix}dtr_daily_summary_lf s ON (s.employee_id = u.employee_id)
			INNER JOIN {$this->db->dbprefix}user_company_department d ON (u.department_id = d.department_id)
			INNER JOIN {$this->db->dbprefix}employee e ON (u.employee_id = e.employee_id)
		 	JOIN {$this->db->dbprefix}employee_dtr r
		    	ON (r.employee_id = s.employee_id AND r.date = s.date)
	    	LEFT JOIN {$this->db->dbprefix}employee_obt obt
				ON (obt.employee_id = u.employee_id
					AND DATE(obt.date_approved) BETWEEN '".  $startcutoff . " 00:00:00' AND '" . $p->cutoff . " 23:59:59'
					AND obt.form_status_id = 3) AND (s.date BETWEEN obt.date_from AND obt.date_to)
		 	LEFT JOIN {$this->db->dbprefix}employee_obt_date obt_date
		 		ON obt_date.employee_obt_id = obt.employee_obt_id AND (
					DATE(CONCAT(obt_date.date, ' ', obt.time_start)) BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "'
					AND DATE(CONCAT(obt_date.date, ' ', obt.time_end)) BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "'
				) AND s.date = obt_date.date
			WHERE (s.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "' AND u.deleted = 0)
			AND obt.date_from IS NOT NULL AND s.employee_type = 3

			UNION

			SELECT e.id_number, u.lastname, u.firstname, d.department, s.date, time(r.time_in1) as time_in1, time(r.time_out1) as time_out1,
			u.deleted, u.company_id, u.division_id, u.department_id, u.employee_id, e.employed_date, e.resigned_date,e.status_id,e.employee_type,
			'LEAVE' as Particulars
			FROM {$this->db->dbprefix}user u 
			INNER JOIN {$this->db->dbprefix}dtr_daily_summary_lf s ON (s.employee_id = u.employee_id)
			INNER JOIN {$this->db->dbprefix}user_company_department d ON (u.department_id = d.department_id)
			INNER JOIN {$this->db->dbprefix}employee e ON (u.employee_id = e.employee_id)
	    	...
	    	LEFT JOIN {$this->db->dbprefix}employee_leaves l
				ON (l.employee_id = u.employee_id
					AND l.application_form_id not in (5,6,7) 
					AND DATE(l.date_approved) BETWEEN '".  $startcutoff . " 00:00:00' AND '" . $p->cutoff . " 23:59:59'
			...
			LEFT JOIN {$this->db->dbprefix}employee_leaves_dates l_date
				ON l_date.employee_leave_id = l.employee_leave_id AND (
					l_date.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "'
				) AND s.date = l_date.date
			WHERE (s.date BETWEEN '". $prevcutof->date_from . "' AND '" . $prevcutof->date_to . "' AND u.deleted = 0)
			AND l.date_from IS NOT NULL AND s.employee_type = 3
		) as t_max";
		...
		$sql .= " date >= employed_date ORDER BY 1";
...
/*	        $start = $limit * $page - $limit;
	        ...
	        foreach ($result->result() as $row) {
	            ...
	            $response->rows[$ctr]['cell'][6] = $row->{"Particulars"};
	            $ctr++;
	        }
	    }
    ...
    function _set_listview_query($listview_id = '', $view_actions = true) {
        $this->listview_column_names = array('Employee Id', 'Employee Name', 'Department', 'Date', 'Time In', 'Time Out','Particulars');
...
/*SELECT date AS date,employee_id,'OVERTIME' as type FROM {$this->db->dbprefix}employee_oot 
			WHERE 
				date_approved BETWEEN '". $period->date_from . " 00:00:00' AND '" . $period->date_to . " 00:00:00'
				AND (date NOT BETWEEN '". $period->date_from . "' AND '" . $period->date_to . "')
				AND date BETWEEN '". $prev_period->date_from ."' AND '".$prev_period->date_to."'
	...
	private function _excel_export($record_id = 0)
	{
        ...
        if ($this->input->post('sidx')) {
            ...
            $sql .= ' ORDER BY `' . $sidx . '` ' . $sord;
        	 ...
        	 $sql .= ' ORDER BY CONCAT(lastname, \', \',lastname), date';	
        }
		...
		$objPHPExcel->getProperties()->setTitle("Compliance Report")
		...
		$activeSheet->setCellValueExplicit('A2', 'Compliance Report', PHPExcel_Cell_DataType::TYPE_STRING); 
		if( $this->input->post('period_date') ){
			$activeSheet->setCellValueExplicit('A3', date('F d,Y',strtotime($p->date_from)).' - '.date('F d,Y',strtotime($p->date_to)), PHPExcel_Cell_DataType::TYPE_STRING); 
		}
		...
		header('Content-Disposition: attachment;filename=' . date('Y-m-d').'-'.url_title("Compliance Report").'.xls');
