...
class Batch_entry extends MY_Controller
{
	...
	function index(){
        ...
        $tabs[] = '<li class="active"><a href="javascript:void(0)">Batch Entries</li>';
	...
	function ajax_save(){
		...
		//employees
		$this->db->delete('payroll_batch_entry_employee', array( $this->key_field => $this->key_field_val ));
		...
		foreach( $employees as $index => $employee_id ){
			$data = array(
				...
				'unit_rate' => str_replace(',', '', $unit_rate[$index]),
				'amount' => str_replace(',', '', $amount[$index])
			);
			$this->db->insert( 'payroll_batch_entry_employee', $data );
		}
	}
