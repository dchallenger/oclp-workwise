...
class Employee_loan extends MY_Controller
{
	...
	function index(){
        ...
        $tabs[] = '<li><a href="'.base_url('payroll/dtr_summary').'">DTR Summary</li>';
       	$tabs[] = '<li class="active"><a href="javascript:void(0)">Employee Loans</li>';
	...
	function ajax_save(){
		...
		if( $this->input->post('record_id') && $record_id == "-1" ){
			$_POST['entry_date'] = date('Y-m-d');
		}
		else{
			unset( $_POST['entry_date'] );
		}
	...
	function get_loan_default(){
		$response = $this->db->get_where('payroll_loan', array('loan_id' => $this->input->post('loan_id')))->row_array();
