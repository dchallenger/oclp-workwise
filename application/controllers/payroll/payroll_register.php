...
class payroll_register extends MY_Controller
{
    ...
    function export_report(){
        ...
        switch ($_POST['report_type_id']){
            //summary
            case '0':
                $html = $this->export_summary($paycode_id, $code_status_id, $payroll_date, "PAYROLL REGISTER" );
                break;            
            //summary total
            case '1':
                $html = $this->export_summary_total($paycode_id, $code_status_id, $payroll_date, "PAYROLL REGISTER TOTAL" );
                break;
        }
    ...
    function export_summary($paycode_id, $code_status_id, $payroll_date, $title){
        ...
        $tot_no_emp = $this->db->query("SELECT distinct pct.employee_id FROM {$this->db->dbprefix}user u
                                    LEFT JOIN {$this->db->dbprefix}payroll_closed_transaction pct on u.employee_id = pct.employee_id
                                    WHERE 1 AND payroll_date = '{$payroll_date}' AND pct.transaction_code = 'netpay'
        ...
        if( $proj_cnt > 0 ){
            ...
            foreach ($proj_record as $key => $proj) {
                ...
                $qry = $this->db->query("SELECT distinct u.employee_id
                                    ...
                                    LEFT JOIN {$this->db->dbprefix}payroll_closed_transaction pct on u.employee_id = pct.employee_id
                                    WHERE pct.cost_code = '{$proj->cost_code}' AND payroll_date = '{$payroll_date}' AND pct.transaction_code = 'netpay'
                                        $pay_code $additional_qry $code_status
                ...
                if($total_no_employees > 0){
                    for($i=1;$i<=$number_of_page; $i++){  
                        ...
                        $dtl = $this->db->query("SELECT distinct u.employee_id, id_number,  lastname, firstname, middleinitial, aux, salary, bank_acct
                                    ...
                                    LEFT JOIN {$this->db->dbprefix}employee ee ON u.employee_id = ee.employee_id
                                    LEFT JOIN {$this->db->dbprefix}payroll_closed_transaction pct on u.employee_id = pct.employee_id
                                    WHERE pct.cost_code = '{$proj->cost_code}'AND payroll_date = '{$payroll_date}' $pay_code $additional_qry $code_status
                                    ORDER BY lastname, firstname, middleinitial
                        ...
                        foreach ($dtl->result() as $key => $dtl_res) {
                            ...
                            $salary = $this->db->query(" SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction 
                                                WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('SALARY') AND employee_id = $dtl_res->employee_id")->row();
                            ...
                            $ovrtme = $this->db->query("SELECT  SUM(amount) as amount
                                                ...
                                                WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1,2) AND pct.transaction_code != 'salary' 
                                                    AND pt.transaction_class_id = 10 AND pt.transaction_code NOT LIKE ('%ND%') AND employee_id = $dtl_res->employee_id ")->row();

                            $nd = $this->db->query("SELECT  SUM(amount) as amount
                                                FROM {$this->db->dbprefix}payroll_closed_transaction pct
                                                LEFT JOIN {$this->db->dbprefix}payroll_transaction pt ON pct.transaction_id = pt.transaction_id
                                                WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1,2) AND pct.transaction_code != 'salary' 
                                                    AND pt.transaction_class_id = 10 AND pt.transaction_code LIKE ('%ND%') AND employee_id = $dtl_res->employee_id ")->row();

                            $non_tax = $this->db->query("SELECT  SUM(amount) as amount
                                                FROM {$this->db->dbprefix}payroll_closed_transaction pct
                                                LEFT JOIN {$this->db->dbprefix}payroll_transaction pt ON pct.transaction_id = pt.transaction_id
                                                WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (2,6,7) AND employee_id = $dtl_res->employee_id ")->row();
                            ...
                            $taxable = $this->db->query("SELECT  SUM(amount) as amount
                                                ...
                                                WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1) AND pct.transaction_code != 'salary' 
                                                    AND pt.transaction_class_id != 10 AND employee_id = $dtl_res->employee_id ")->row();

                            $attnd_ded = $this->db->query("SELECT  SUM(amount) as amount
                                                FROM {$this->db->dbprefix}payroll_closed_transaction pct
                                                LEFT JOIN {$this->db->dbprefix}payroll_transaction pt ON pct.transaction_id = pt.transaction_id
                                                WHERE payroll_date = '{$payroll_date}' AND pct.transaction_code IN ('LWOP','ABSENCES','DEDUCTION_LATE','DEDUCTION_UNDERTIME')
                                                    AND employee_id = $dtl_res->employee_id ")->row();

                            $gross  = ( $salary->amount + $ovrtme->amount + $nd->amount + $taxable->amount ) - $attnd_ded->amount;
                            ...
                            $whtax = $this->db->query(" SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction
                                                        WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('WHTAX') AND employee_id = $dtl_res->employee_id ")->row();

                            $sss = $this->db->query("   SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction
                                                        WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('SSS_EMP') AND employee_id = $dtl_res->employee_id ")->row();

                            $philhealth = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction
                                                        WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('PHIC_EMP') AND employee_id = $dtl_res->employee_id ")->row();

                            $pagibig = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction
                                                        WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('HDMF_EMP') AND employee_id = $dtl_res->employee_id ")->row();

                            $dedtn = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
                                                LEFT JOIN {$this->db->dbprefix}payroll_transaction pt on pct.transaction_id = pt.transaction_id
                                                WHERE payroll_date = '{$payroll_date}' AND transaction_class_id IN (24,25,28)
                                                    AND employee_id = $dtl_res->employee_id ")->row();

                            $loan = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
                                                LEFT JOIN {$this->db->dbprefix}payroll_transaction pt on pct.transaction_id = pt.transaction_id
                                                WHERE payroll_date = '{$payroll_date}' AND pt.transaction_type_id IN (3,4,5) AND pt.transaction_class_id = 26 AND employee_id = $dtl_res->employee_id ")->row();

                            $tot_ded = $loan->amount + $dedtn->amount + $whtax->amount + $philhealth->amount + $sss->amount + $pagibig->amount;

                            $netpay = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction
                                                WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('NETPAY') AND employee_id = $dtl_res->employee_id ")->row();
                        ...
                        if($tot_no_emp == $emp_count){
                            ...
                            <table>
                                ...
                                <tr>
                                    <td style=" width:10% ; text-align:right ; border-left-width:3px  ; ">GRAND TOTAL </td>
    ...
    // function export_summary_total($cost_code, $paycode_id, $company_id, $employee_id, $payroll_date, $title){
    ...
    //     $dtl = $this->db->query("SELECT distinct u.employee_id, id_number,  lastname, firstname, middleinitial, salary, bank_acct
    //                 FROM {$this->db->dbprefix}user u
    ...
    //                 WHERE w.cost_code = '{$proj->cost_code}'AND payroll_date = '{$payroll_date}'  and w.assignment = 1  $pay_code 
    //                 ORDER BY lastname, firstname, middleinitial");
    ...
    //     $total_salary = $this->db->query(" SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    ...
    //                         WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('SALARY')  and w.assignment = 1 $pay_code")->row();
    ...
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1,2) AND pct.transaction_code != 'salary' 
    //                             AND pt.transaction_class_id = 10 AND pt.transaction_code NOT LIKE ('%ND%') and w.assignment = 1  $pay_code")->row();
    ...
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1,2) AND pct.transaction_code != 'salary' 
    //                             AND pt.transaction_class_id = 10 AND pt.transaction_code LIKE ('%ND%') and w.assignment = 1  $pay_code")->row();
    ...
    //                         LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (2,6,7) and w.assignment = 1  $pay_code")->row();
    ...
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pct.transaction_type_id IN (1) AND pct.transaction_code != 'salary' 
    //                             AND pt.transaction_class_id != 10  and w.assignment = 1 $pay_code")->row();

    //     $tot_attnd_ded = $this->db->query("SELECT  SUM(amount) as amount
    //                         FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                         LEFT JOIN {$this->db->dbprefix}payroll_transaction pt ON pct.transaction_id = pt.transaction_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pct.transaction_code IN ('LWOP','ABSENCES','DEDUCTION_LATE','DEDUCTION_UNDERTIME')
    //                             and w.assignment = 1 $pay_code")->row();

    //     $total_gross  = ( $total_salary->amount + $total_ovrtme->amount + $total_nd->amount + $total_taxable->amount ) - $tot_attnd_ded->amount;

    //     $total_inc = $total_gross + $total_non_tax->amount;

    //     $total_whtax = $this->db->query(" SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                                 LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                                 LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                                 WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('WHTAX') and w.assignment = 1  $pay_code")->row();

    //     $total_sss = $this->db->query("   SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                                 LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                                 LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                                 WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('SSS_EMP') and w.assignment = 1  $pay_code")->row();

    //     $total_philhealth = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                                 LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                                 LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                                 WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('PHIC_EMP') and w.assignment = 1  $pay_code")->row();

    //     $total_pagibig = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                                 LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                                 LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                                 WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('HDMF_EMP') and w.assignment = 1  $pay_code")->row();

    //     $total_dedtn = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                         LEFT JOIN {$this->db->dbprefix}payroll_transaction pt on pct.transaction_id = pt.transaction_id 
    //                         LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pt.transaction_type_id IN (3,4,5) AND pt.transaction_class_id != 26 
    //                             AND pt.transaction_code IN ('TAXABLE_DEDUCTION')  and w.assignment = 1  $pay_code")->row();

    //     $total_loan = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                         LEFT JOIN {$this->db->dbprefix}payroll_transaction pt on pct.transaction_id = pt.transaction_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND pt.transaction_type_id IN (3,4,5) AND pt.transaction_class_id = 26 
    ...
    //     $total_netpay = $this->db->query("SELECT sum(amount) as amount FROM {$this->db->dbprefix}payroll_closed_transaction pct
    //                         LEFT JOIN {$this->db->dbprefix}employee_work_assignment w on pct.employee_id = w.employee_id
    //                         LEFT JOIN {$this->db->dbprefix}employee_payroll e on pct.employee_id = e.employee_id
    //                         WHERE payroll_date = '{$payroll_date}' AND transaction_code IN ('NETPAY') and w.assignment = 1  $pay_code")->row();
    //     $xcel_hed .= ' 
    //             <tr><td></td></tr>
    //             <tr>
    //                 <td style=" width:15% ; font-size: 2;  border-top-width:3px ; border-left-width:3px ; "> </td>
