...
class Headcount_report extends MY_Controller
{
    ...
    { 	
		...
		$data['jqgrid'] = 'recruitment/analytics/jqgrid';
	...
	function listview()
	{
		...
		if( $this->db->_error_message() != "" ){
	        ...
	        foreach ($result->result() as $row) { 
				...
				$existing = $this->get_incumbent($year, $row->division_id);
				$prev_inc = $this->get_incumbent($prev_year, $row->division_id);
				$budgeted 		  = $this->get_budgeted($year, $row->division_id);
		        ...
		        $response->rows[$ctr]['cell'][3] = $prev_inc;
				$ctr++;
    ...
    function _set_listview_query($listview_id = '', $view_actions = true) {
    	$prev_year = date('Y')-1;
    	$curr_year = date('Y');
    	$dec31   = $prev_year. '-' . date('12-01');

    	$last_year = date('F d, Y', strtotime($dec31));
   
   		$this->listview_column_names = array('Division','Actual', 'Budgeted', 'Actual');
	...
	function export() {	
		...
		$fields = array('Division','Actual', 'Budgeted', 'Actual');
		...
		$activeSheet->setCellValue('A2', $year.'Headcount Report');
		...
		$fields2 = array('division', 'actual', 'budgeted', 'as_of');
		
		foreach ($results->result() as $key => $result) {
			$sub_ctr   = 0;			
			$alpha_ctr = 0;

			$existing = $this->get_incumbent($year, $result->division_id);
			$prev_inc = $this->get_incumbent($prev_year, $result->division_id);
			$budgeted 		  = $this->get_budgeted($year, $result->division_id);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D' . $line, ($prev_inc) ? $prev_inc : '0');
