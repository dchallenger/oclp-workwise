...
class Training_bond_schedule extends MY_Controller
{
	function __construct()
    {
		...
		$this->listview_title = 'Training Bond Schedule';
		$this->listview_description = 'This module lists all defined Training Bond Schedule(s).';
		$this->jqgrid_title = "Training Bond Schedule List";
		$this->detailview_title = 'Training Bond Schedule Info';
		$this->detailview_description = 'This page shows detailed information about a particular Training Bond Schedule.';
		$this->editview_title = 'Training Bond Schedule Add/Edit';
		$this->editview_description = 'This page allows saving/editing information about Training Bond Schedule(s).';
    }
				...
				if($result->num_rows() > 0){
					...
					foreach ($result->result_array() as $row){
						...
						foreach($this->listview_columns as $column => $detail){
							...
							elseif( $detail['name'] == 'rls_months' ){

								$cell[$cell_ctr] = floor($row[$detail['name']]).' Months';
								$cell_ctr++;

							}
							elseif( $detail['name'] == 'rls_days' ){

								$cell[$cell_ctr] = floor($row[$detail['name']]).' Days';
								$cell_ctr++;
	...
	{	
		...
		$this->db->where('( cost BETWEEN "'.str_replace(',', '', $this->input->post('training_cost_from')).'" AND "'.str_replace(',', '', $this->input->post('training_cost_to')).'" )');
		$this->db->where('deleted',0);
		$this->db->where('training_bond',1);
		$training_course_result = $this->db->get('training_subject');

		if( $training_course_result->num_rows() > 0 ){

			foreach( $training_course_result->result_array() as $training_course_info ){

				$data = array(
					'rls' => $this->input->post('rls_days')
				);

				$this->db->where('training_subject_id',$training_course_info['training_subject_id']);
				$this->db->update('training_subject',$data);
	...
	function convert_rls_days_to_months(){
		
		$rls_months = round( ( $this->input->post('rls_days') / 30) , 0, PHP_ROUND_HALF_DOWN);

		$response->rls_months = $rls_months;
		$this->load->view('template/ajax', array('json' => $response));

	}

	function validate_existing_bond(){

		$training_cost_from = str_replace(',', '', $this->input->post('training_cost_from'));
		$training_cost_to = str_replace(',', '', $this->input->post('training_cost_to'));

		if( $training_cost_from > $training_cost_to ){

			$response->validate_type = 1;
			$response->msg_type = "error";
			$response->msg = "Actual Cost From must be less than or equal to Actual Cost To.";

		}
		else{

			$sql = 'SELECT * FROM '.$this->db->dbprefix('training_bond_schedule').' 
			WHERE ((( '.$training_cost_from.' BETWEEN training_cost_from AND training_cost_to) OR ( '.$training_cost_to.' BETWEEN training_cost_from AND training_cost_to))
			OR (( training_cost_from BETWEEN '.$training_cost_from.' AND '.$training_cost_to.') OR ( training_cost_to BETWEEN '.$training_cost_from.' AND '.$training_cost_to.')))
			AND training_bond_schedule_id != '.$this->input->post('record_id').' AND deleted = 0';

			$training_bond_schedule = $this->db->query($sql);

			if( $training_bond_schedule->num_rows() > 0 ){
				$response->validate_type = 2;
				$response->msg_type = "error";
				$response->msg = "Actual Cost range must not overlap with the existing bond.";
			}
			else{
				$response->validate_type = 0;
