...
class Training_feedback extends MY_Controller
{
	function __construct()
    {
		...
		$this->listview_title = 'Training Feedback';
		$this->listview_description = 'This module lists all defined training feedback(s).';
		$this->jqgrid_title = "Training Feedback List";
		$this->detailview_title = 'Training Feedback Info';
		$this->detailview_description = 'This page shows detailed information about a particular training feedback.';
		$this->editview_title = 'Training Feedback Add/Edit';
		$this->editview_description = 'This page allows saving/editing information about training feedback(s).';
    
		if(!$this->user_access[$this->module_id]['post'] && !$this->user_access[$this->module_id]['approve'] ){
			$this->filter = "training_feedback.employee_id = ".$this->userinfo['user_id'];
		}
	...
	function listview()
	{
		...
		if( $this->user_access[$this->module_id]['post'] == 0 && $this->user_access[$this->module_id]['approve'] == 1 ){
			...
			if( $participant_subordinate_result->num_rows() > 0 ){
				foreach( $participant_subordinate_result->result() as $subordinate_info ){
					array_push($training_feedback_list, $subordinate_info->training_calendar_id);
				}
			}
		...
		$this->db->join('training_feedback','training_feedback.training_calendar_id = '.$this->module_table.'.training_calendar_id','left');
		$this->db->where($this->module_table.'.deleted = 0 AND '.$search);
		$this->db->group_by($this->module_table.'.training_calendar_id');
		if( $this->user_access[$this->module_id]['post'] == 0 && $this->user_access[$this->module_id]['approve'] == 1 ){
			if( count($training_feedback_list) != 0 ){
				$this->db->where_in($this->module_table.'.training_calendar_id',$training_feedback_list);
			}
			else{
	...
	function _default_grid_actions($module_link = "", $container = "", $record = array()) {
		...
		if ($this->user_access[$this->module_id]['view'] && $feedback_participant) {
			$actions .= '<a class="icon-button icon-16-info view_feedback" tooltip="View Feedback" onclick="" module_link="training/training_feedback_participants" tooltip="Delete" href="javascript:void(0)"></a>';
		}

		if ($this->user_access[$this->module_id]['edit'] && $feedback_participant) {
			$actions .= '<a class="icon-button icon-16-edit edit_feedback" tooltip="Edit Feedback" onclick="" module_link="training/training_feedback_participants" href="javascript:void(0)"></a>';
		}
	...
	function get_feedback_participant_id(){

		$calendar_id = $this->input->post('calendar_id');

		$training_feedback = $this->db->get_where('training_feedback',array('training_calendar_id'=>$calendar_id, 'employee_id'=>$this->userinfo['user_id']));

		if( $training_feedback->num_rows() > 0 ){
			$training_feedback = $training_feedback->row();
			$response->feedback_id = $training_feedback->feedback_id;
		}
		else{
