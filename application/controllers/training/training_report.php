...
class Training_report extends MY_Controller
{
	...
	function index()
    {
		...
		$data['content'] = 'training/training_report/listview';
		$data['jqgrid'] = 'training/training_report/jqgrid';
					...
					foreach ($result->result_array() as $row){
						...
						foreach($this->listview_columns as $column => $detail){
							...
							elseif( $detail['name'] == 'employee_id' ){
								...
								$this->db->where('training_employee_database.employee_id',$row['employee_id']);
								...
								$cell[$cell_ctr] = $total_training_cost;
								$cell_ctr++;
	...
	function export() {	
		...
		$this->db->select('employee.id_number as "Employee ID Number"');
		$this->db->select('CONCAT( '.$this->db->dbprefix('user').'.firstname, " ", '.$this->db->dbprefix('user').'.lastname ) as "Employee Name"',false);
		$this->db->select('user_position.position as "Position"');
		$this->db->select('user_company_division.division as "Division"');
		$this->db->select('user_company_department.department as "Department"');
		$this->db->select('group_name.group_name as "Group"');
		$this->db->select('project_name.project_name as "Project"');
		$this->db->select('user_rank.job_rank as "Rank"');
		$this->db->select('employment_status.employment_status as "Employment Status"');
		$this->db->select('employee.employee_id as "Total Training Cost"');
		$this->db->select('employee.total_training_running_balance as "Total Running Balance"');
		$this->db->select($this->module_table.'.daily_training_cost as "Daily Training Cost"');
		$this->db->select('training_subject.training_subject as "Training Course"');
		$this->db->select('training_provider.training_provider as "Provider"');
		$this->db->select('training_calendar.venue as "Venue"');
		$this->db->select($this->module_table.'.start_date as "Start Date"');
		$this->db->select($this->module_table.'.end_date as "End Date"');
		$this->db->select('training_subject_schedule.training_subject_schedule as "Schedule"');
		$this->db->select('training_calendar.cost_per_pax as "Training Cost"');
		$this->db->select($this->module_table.'.bond_start_date as "Bond Start Date"');
		$this->db->select($this->db->dbprefix($this->module_table).'.bond_end_date as "Bond End Date"');
		$this->db->select($this->module_table.'.no_bond_days as "No. of Bond Days"');
		$this->db->select('IF( CURRENT_DATE() > DATE('.$this->db->dbprefix($this->module_table).'.bond_end_date), "Fullfiled", "On-going" ) as "Bond Status"',false);
		$this->db->select('training_subject.rls as "Required Length of Service"');
		$this->db->select('IF( '.$this->db->dbprefix('training_subject').'.training_bond = 1, "Yes", "No" ) as "With Training Bond"',false);
		$this->db->from($this->module_table);
		...
		$this->db->join('group_name','group_name.group_name_id = '.$this->module_table.'.group_name_id','left');
	...
	private function _excel_export()
	{
		...
		$activeSheet->setCellValueExplicit('A2', 'Training Report', PHPExcel_Cell_DataType::TYPE_STRING); 
		...
		foreach ($query->result() as $row) {
			...
			foreach ($fields as $field) {
				...
				if( $field == "Total Training Cost" ){

					$this->db->join('training_calendar','training_calendar.training_calendar_id = training_employee_database.training_calendar_id','left');
					$this->db->where('training_employee_database.employee_id', $row->{$field});
					$training_database_result = $this->db->get('training_employee_database');
					...
					$objPHPExcel->getActiveSheet()->setCellValueExplicit($xcoor . $line, $total_training_cost, PHPExcel_Cell_DataType::TYPE_STRING); 

				}
				elseif( $field == "Start Date" || $field == "End Date" || $field == "Bond Start Date" || $field == "Bond End Date" ){
		...
		header('Content-Disposition: attachment;filename=Training_Report_'.date('Y-m-d').'xls');
