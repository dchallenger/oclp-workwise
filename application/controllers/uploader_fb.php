...
class uploader_fb extends CI_Controller
{
...
*/
    ...
    //FIRST BALFOUR Employee upload
     function firstbalfour_uploader(){
     	$this->db->trans_start();
	    	$this->load->library('encrypt');
	    	$this->_fb_initial_upload();
	    	$this->_fb_user_upload();
	    	$this->_fb_employee_upload();
	    	$this->_fb_dtr_setup_upload();
	    	$this->_fb_leave_balance_upload();
    ...
    private function _fb_initial_upload(){
    	echo 'Initial upload to employee and user table<br/>';
		$fdata = file('uploads/firstbalfour/fb/fb_employee_employed_date.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				$insert['employed_date'] = (isset($insert['employed_date']) && strtotime($insert['employed_date'])) ? date('Y-m-d', strtotime($insert['employed_date'])) : '0000-00-00';

				if( $emp->num_rows() == 1 ){
					$emp = $emp->row();
					$this->db->update('employee', $insert, array('employee_id' => $emp->employee_id));
					echo "update!!!-> $id_number <br>";
					$count++;
				}
				else if($emp->num_rows() > 1){
					echo 'Too many data found for user with id number : '.$id_number.'<br/>';
    ...
    private function _fb_user_upload(){
    	echo 'Users table<br/>';
		$fdata = file('uploads/firstbalfour/fb/fb_user.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		...
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				$insert['birth_date'] = (isset($insert['birth_date']) && strtotime($insert['birth_date'])) ? date('Y-m-d', strtotime($insert['birth_date'])) : null;

				$insert['date_of_marriage'] = (isset($insert['date_of_marriage']) && strtotime($insert['date_of_marriage'])) ? date('Y-m-d', strtotime($insert['date_of_marriage'])) : null;
    ...
    private function _fb_employee_upload(){
    	echo 'Users table<br/>';
		$fdata = file('uploads/firstbalfour/fb/fb_employee.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		...
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				$insert['employed_date'] = (isset($insert['employed_date']) && strtotime($insert['employed_date'])) ? date('Y-m-d', strtotime($insert['employed_date'])) : '0000-00-00';
				$insert['original_hired_date'] = (isset($insert['original_hired_date']) && strtotime($insert['original_hired_date'])) ? date('Y-m-d', strtotime($insert['original_hired_date'])) : null;
				$insert['end_date'] = (isset($insert['end_date']) && strtotime($insert['end_date'])) ? date('Y-m-d', strtotime($insert['end_date'])) : null;
    ...
    private function _fb_dtr_setup_upload(){
    	echo 'DTR Setup table<br/>';
		$fdata = file('uploads/firstbalfour/fb/fb_dtr_setup.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				$emp = $this->db->get_where('employee', array('deleted' => 0, 'id_number' => $id_number));
				$emp2 = $emp->row();
				$dtr_set = $this->db->get_where('employee_dtr_setup', array('deleted' => 0, 'employee_id' => $emp2->employee_id));
				if( $emp->num_rows() == 1 && $dtr_set->num_rows() == 1){
						$this->db->update('employee_dtr_setup', $insert, array('employee_id' => $emp2->employee_id));
						$count++;
				}
				else if($emp->num_rows() > 1){
					echo 'Too many data found for user with id number : '.$id_number.'<br/>';
					...
					//insert
					$this->db->insert('employee_dtr_setup', array('employee_id' => $emp2->employee_id,'shift_calendar_id' => $insert['shift_calendar_id'])); 
					$count++;
				}					
    ...
    private function _fb_leave_balance_upload(){
    	echo 'Leave Balance table<br/>';
		$fdata = file('uploads/firstbalfour/fb/fb_leave_balance.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				$emp = $this->db->get_where('employee', array('deleted' => 0, 'id_number' => $id_number));
				$emp2 = $emp->row();
				$leave_bal = $this->db->get_where('employee_leave_balance', array('deleted' => 0, 'employee_id' => $emp2->employee_id, 'year' => $insert['year']));
				if( $emp->num_rows() == 1 && $leave_bal->num_rows() == 1){
						$this->db->update('employee_leave_balance', $insert, array('employee_id' => $emp2->employee_id));
						$count++;
				}
				else if($emp->num_rows() > 1){
					echo 'Too many data found for user with id number : '.$id_number.'<br/>';
					...
					//insert
					$this->db->insert('employee_leave_balance', array('employee_id' => $emp2->employee_id,'year' => $insert['year'])); 
					$count++;
				}					
    ...
    //FIRST BALFOUR Employee upload
     function firstbalfour_bio_work_uploader(){
     	$this->db->trans_start();
	    	$this->load->library('encrypt');
	    	$this->_fb_biometrics_upload();
	    	$this->_fb_work_assignment_upload();
		$this->db->trans_complete() ;
    ...
    private function _fb_biometrics_upload(){
    	echo 'BIOMETRICS Setup table<br/>';
		$fdata = file('uploads/firstbalfour/bio/fb_biometrics.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				if( $emp->num_rows() == 1 || $dtr_set->num_rows() == 1){
						...
						$this->db->update('employee', $insert, array('employee_id' => $emp2->employee_id));
						// echo $this->db->last_query()."<br>";
						$count++;
				}
				else if($emp->num_rows() > 1){
					...
					echo 'Employee not yet inserted : '.$id_number.'<br/>';
				}
				// else{
				// 	//insert
				// 	$this->db->insert('employee_dtr_setup', array('employee_id' => $emp2->employee_id,'shift_calendar_id' => $insert['shift_calendar_id'])); 
    ...
    private function _fb_work_assignment_upload(){
    	echo 'WORK ASSIGNMENT Setup table<br/>';
		$fdata = file('uploads/firstbalfour/bio/fb_work_assignment.txt');
		$skip = true;
		$count = 0;
		$count_row = 0;
		foreach($fdata as $row){
			...
			if(!$skip){
				...
				// $insert['start_date'] = (isset($insert['start_date']) && strtotime($insert['start_date'])) ? date('Y-m-d', strtotime($insert['start_date'])) : null;

				// $insert['end_date'] = (isset($insert['end_date']) && strtotime($insert['end_date'])) ? date('Y-m-d', strtotime($insert['end_date'])) : null;

				$emp = $this->db->get_where('employee', array('deleted' => 0, 'id_number' => $id_number));
				$emp2 = $emp->row();
				$insert_condition = $insert;
				$insert_condition['deleted'] = 0;
				$insert_condition['employee_id'] = $emp2->employee_id;

				// $dtr_set = $this->db->get_where('employee_work_assignment', $insert_condition);
				$dtr_set = $this->db->get_where('user', array('deleted' => 0, 'employee_id' => $emp2->employee_id));
				// $insert['date_created'] = date('Y-m-d') ;

				if( $emp->num_rows() == 1 && $dtr_set->num_rows() == 1){
					$insert['employee_id'] = $emp2->employee_id;
						$this->db->update('user', $insert, array('employee_id' => $emp2->employee_id));
						// echo $this->db->last_query()."<br>";
						$count++;
				}
				else if($emp->num_rows() > 1){
				...
				// 	$insert['employee_id'] = $emp2->employee_id;
				// 	//delete
				// 	$this->db->where('employee_id', $emp2->employee_id);
				// 	$this->db->delete('employee_work_assignment');
				// 	//insert
				// 	$this->db->insert('employee_work_assignment', $insert); 
				// 		// echo $this->db->last_query()."<br>";
				// 	$count++;
				// }					
			 ...
			 // $this->db->where('employee_id', 'NULL');
			 // $this->db->delete('employee_work_assignment');
				foreach( $data as $index => $value ){
					if( !empty($value) ) $index_key[$index] = trim($value);
				}
