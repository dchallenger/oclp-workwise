<input type="hidden" name="applicant_id" value="<?=$applicant_id?>" id="applicant_id">
<input type="hidden" name="reference_ids" value="<?=$reference_ids?>" id="reference_ids">
...
<div id="fg-<?=$cnt?>" class="" fg_id="<?=$cnt?>" style="margin-left:2%">
	...
	<div class="col-1-form hidden">
		<?php foreach ($ref_q['questionnaires']['questions'] as $questions => $values): ?>
			<h6><?=$values?>:</h6> 
			<div class="col-1-form ">
			<?php  
				foreach ($ref_q['questionnaires'][$questions] as $key => $question):?>
				<div class="form-item view odd ">
				    <label class="label-desc view gray" for="question_description"><?=$question->description?>:</label>
				    <div class="text-input-wrap"><?=$record_questions[$questions][$reference->record_id][$key]?></div>		
				</div>
			<?php endforeach;?>
			</div>
		<?php endforeach;?>
