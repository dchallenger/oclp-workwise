    ...
    <?php if( isset($this->user_access[$this->module_id]['post']) && $this->user_access[$this->module_id]['post'] == 1 ){?>
        <div class="by_employee" style="float:left; width:46%">
            <div style="float:left;">Employee:</div>
            <div style="float:left; width:325px" class="form-item">
                <?php        
                    ...
                    $employees =  $this->db->get('user')->result_array();

                    $options = array();
                    foreach($employees as $employee):
                        $options[$employee['department']][$employee['user_id']] = $employee['lastname'].', '.$employee['firstname'];
                    endforeach;
                    $custom = 'id="employee_id"';
                    echo form_multiselect('employee_id', $options,  '', $custom);
                ?>
            </div>
        </div>
        <div style="float:left;padding-left:5px"><button onclick="get_data();" id="filter-dtr">Filter</button></div>
        <div class="clear"></div>
    <?php }?>
