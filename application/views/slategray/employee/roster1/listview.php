...
?>
    ...
    <?  else :?>
       <form id="export-form" method="post" action="">
          <input type="hidden" name="export_link" id="export_link" value="<?=site_url('employee/roster1/export')?>" />
          <input type="hidden" name="previous_page" id="previous_page" value="<?=base_url().$this->module_link?>/detail"/>        
          <div id="form-div">
            <div class="col-1-form"> 
                <div class="form-item odd ">
                    <label class="label-desc gray" for="department" id="category_selected">Department:</label>
                    <div class="multiselect-input-wrap">
                        <select multiple="multiple" class="multi-select" style="width:400px;" name="department[]" id="department">
                            <?php
                                foreach($department as $field){ ?>
                                <option value="<?php echo $field['department_id'] ?>"><?php echo $field['department'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
