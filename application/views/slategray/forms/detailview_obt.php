                ...
                <div class="<?= !empty($fieldgroup['layout']) ? 'col-1-form' : 'col-2-form' ?> view"> <?php
                    ...
                    if (isset($fieldgroup['fields'])) :
                        foreach ($fieldgroup['fields'] as $field) :
                            ...
                            // Dates affected.
                            if ($field['fieldname'] == 'label-dates-affected' && isset($dates_affected)) {
                                echo '<div class="form-item view even ">
                                        <label class="label-desc view gray" for="dates-affected">Inclusive Date:</label>
                                        <div class="text-input-wrap">';
                                echo '<div class="" id="div-detail-dates-affected"><div>';                        
                                foreach ($dates_affected as $date_affected) {   
                                    if($date_affected['cancelled'] == 0) {                         
                                        echo $date_affected['date'] . ' - ' . date('h:i a',strtotime($date_affected['time_start'])).'&nbsp;&nbsp;to&nbsp;&nbsp;'.date('h:i a',strtotime($date_affected['time_end'])). '<br/>';
                                    } else {
                                        echo $date_affected['date'] . ' - ' . date('h:i a',strtotime($date_affected['time_start'])).'&nbsp;&nbsp;to&nbsp;&nbsp;'.date('h:i a',strtotime($date_affected['time_end'])). '&nbsp;<span class="red">Cancelled</span>&nbsp<span class="blue" title="'.$date_affected['remarks'].'"><i>Date : '.date("m/d/y",strtotime($date_affected['date_cancelled'])).'</i></span><br/>';
                                    }
                                }                        
