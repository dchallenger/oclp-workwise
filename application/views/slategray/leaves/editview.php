...
<? else : ?>
        ...
        <form class="style2 edit-view" name="record-form" id="record-form" method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="filter" id="filter" value="<?= $this->input->post('filter') ?>"/>
            <input type="hidden" name="related_id" id="related_id" value="<?= $this->input->post('related_id') ?>"/>
                            ...
                            <div class="<?= !empty($fieldgroup['layout']) ? 'col-1-form' : 'col-2-form' ?>" >
                                ...
                                if (isset($fieldgroup['fields']) && sizeof($fieldgroup['fields']) > 0) :
                                    foreach ($fieldgroup['fields'] as $field) :
                                        ...
                                        if ( (!$this->is_admin && !$this->superadmin) 
                                                && $field['column'] == 'employee_id'
                                                && $field['value'] == '') {
                                            if( $_POST['filter'] != "all" ) $field['value'] = $this->userinfo['user_id'];
                                        }
                                        ...
                                        if ($field['column'] == 'label-dates-affected' && isset($dates_affected)) {
                                            $shifts = array(1 => 'Wholeday', 2 => 'First Half of Shift', 3 => 'Second Half of Shift');
                                            echo '<div id="dates-container">';                        
                                        foreach ($dates_affected as $date_affected) {                  
                                                echo '<input type="hidden" name="employee_leave_date_id[]" value="' . $date_affected['employee_leave_date_id'] . '"/>';
                                                echo '<input type="hidden" name="dates[]" value="' . $date_affected['date'] . '"/>';
                                                echo '<div>' . $date_affected['date'] . ' - ' . form_dropdown('shift_id[]', $shifts, $date_affected['duration_id']) . '</div>';
                                            }                        
                                            echo '</div>';
                                        }
