...
?>
    ...
    <div>
        <div style="float: left; padding-right: 10px;">
            Payroll Date: <input type="text" class="date" name="filter-payroll_date" />
        </div>
        <?php
        /*
        $processing_types = $this->db->get_where('payroll_period_processing_type', array('deleted' => 0)); ?>
        <select name="filter-processing_type_id">
            <option value="">Select...</option><?php
            foreach($processing_types->result() as $row){ ?>
                <option value="<?php echo $row->period_processing_type_id?>"><?php echo $row->period_processing_type?></option> <?php
            }?>     
        </select */?>

        <select name="filter-employee_id" style="width:200px;">
