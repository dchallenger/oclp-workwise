...
<div class="form-item odd ">
  ...
  <div class="text-input-wrap">
    ...
    <input type="hidden" name="benefit_label<?php echo $benefit_id?>" value="<?php echo $benefit->benefit?>">
  	<input type="text" class="benefit-field input-text input-medium text-right"disabled="disabled" value="<?php echo isset($value) ? number_format($value, 2, '.', ',') : ''?>" id="" name="benefit[<?php echo $benefit_id?>]" /> 
    <?php if ($benefit->requires_hrs_work == 1):?><input type="text" class="input-text input-small gray hrs-req" disabled="disabled" value="<?php echo isset($hours) && $hours > 0? $hours : 'Hours Required'?>" name="hours[<?php echo $benefit_id?>]" /><?php endif;?>
    <span class="select-input-wrap-mod">
      <select name="units[<?php echo $benefit_id?>]" disabled="disabled">
  	...
  	<!-- <span class="icon-group">  	
  		<a onclick="delete_benefit( $(this),  <?php echo $benefit_id?>)" href="javascript:void(0);" class="icon-button icon-16-minus" readonly="readonly"></a>
  	</span> -->
  </div>
</div>