...
$(document).ready(function (){
    var email_sent = <?=($email_sent) ? $email_sent : 0 ;?>;
    ...
    $('#schedule_type_id').parent().remove();        
    $('label[for="schedule_type_id"]').after('<div class="text-input-wrap"><input id="schedule_type_id" type="hidden" value="<?php echo $schedule_type ?>" name="schedule_type_id"><input type="text" readonly="readonly" class="input-text text-right" name="schedule_type_disp" value="<?php echo $schedule_name ?>"></div>');
    $('label[for="candidate_id"]').after('<div class="text-input-wrap"><input id="candidate_id_fixed" type="hidden" value="<?php echo $candidate_id ?>" name="candidate_id_fixed"><input type="text" readonly="readonly" name="candidate_disp" class="input-text text-right" value="<?php echo $candidate_name ?>"></div>');    
    $('label[for="candidate_id"]').next().next().remove();    

    if ($('#schedule_type_id').val() == 2){
        $('label[for="clinic_name"]').parent('div').show();    
        $('label[for="result"]').parent('div').show();
        $('label[for="result-yes"]').html('Passed');
        $('label[for="result-no"]').html('Failed');
    }    
    else{
        $('label[for="clinic_name"]').parent('div').remove();     
        $('label[for="result"]').parent('div').remove();
    }
});
</script>