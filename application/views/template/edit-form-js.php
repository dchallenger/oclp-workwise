<script type="text/javascript">
	$(document).ready(function(){
				...
				foreach($date as $param) : ?>
					...
					$( 'input[name="<?php echo $param[0]?>-temp"]' ).datepicker({
						...
						onClose: function(dateText) {
							if (dateText != '') {
								if (typeof(<?php echo $param[0]?>_close) == typeof(Function)) {
									<?php echo $param[0]?>_close(dateText);
								}
							}
						}
					}).keyup(function(e) {
					...
					$('input[name="<?php echo $param[0]?>-temp"]').change(function () {
					        if ($(this).val() != '') {
		...
		<?php
			if(isset($hour_min_sec_picker) && sizeof($hour_min_sec_picker) > 0  ):
				foreach($hour_min_sec_picker as $param) : ?>
					$('input[name="<?php echo $param[0]?>"]').timepicker({
                        showAnim: 'slideDown',
                        showHour: true,
                        showMinute: true,
									...
									success: function(data){
										...
										if(data.upload_id != ""){
											...
											if( response_data.file_type == "image" ){
												var img = '<div class="nomargin image-wrap" style="padding-left:10px;padding-bottom:5px"><img id="file-<?php echo $param[0]?>-'+ data.upload_id +'" src="<?php echo base_url()?>'+response_data.path+'" width="100px" ';
												if(module.get_value("module_id") == 137)
													img += 'height="100px';
												img += '">';

												if(module.get_value("module_id") == 137)
													img += '<input class="image-text nomargin multi" style="width:95px" type="textbox" name="multiple_desc['+data.upload_id+']" id="multiple_desc['+data.upload_id+']"/>';

												img += '<div class="image-delete nomargin multi" field="<?php echo $param[0]?>" upload_id="'+ data.upload_id +'"></div></div>';
												$('#<?php echo $param[0]?>-upload-container').append(img);
											}
											else{
												var img = '<div class="image-text nomargin image-wrap" style="padding-left:10px;padding-bottom:5px"><a id="file-<?php echo $param[0]?>-'+ data.upload_id +'" href="<?php echo base_url()?>'+response_data.path+'" width="100px" target="_blank"><img src="<?php echo base_url().$this->userinfo['theme']?>/images/file-icon-md.png" ';
												if(module.get_value("module_id") == 137)
	...
	function validate_form()
	{
		...
		<?php 
			if (isset($date_from_to) && sizeof($date_from_to) > 0):
				foreach ($date_from_to as $param):?>
					date_from = '<?=$param[0]?>_from';
					date_to   = '<?=$param[0]?>_to';

					validate_date_from("<?php echo $param[0]?>", "<?php echo $param[1]?>");
		<?php 	endforeach;
			endif;
		?>
		...
		<?php 
			if (isset($datetime_from_to_picker) && sizeof($datetime_from_to_picker) > 0):
				foreach ($datetime_from_to_picker as $param):?>
					datetime_from = '<?=$param[0]?>_from';
					datetime_to   = '<?=$param[0]?>_to';

					validate_date_from("<?php echo $param[0]?>", "<?php echo $param[1]?>", datetime_from, datetime_to);
		<?php 	endforeach;
			endif;
		?>
