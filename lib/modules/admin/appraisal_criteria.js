$(document).ready(function() {
	 $('label[for="competency_master_id"]').parent().hide();

	  if( $('label[for="is_core"]').parent().find('input[value="1"]').attr('checked') == 'checked' ){
            
            $('label[for="competency_master_id"]').parent().show();

        }else{
        	$('label[for="competency_master_id"]').parent().hide()
        }

        $('input[name="is_core"]').live('click',function(){
        	if( $('label[for="is_core"]').parent().find('input[value="1"]').attr('checked') == 'checked' ){
            	$('label[for="competency_master_id"]').parent().show();
	        }else{
	        	$('label[for="competency_master_id"]').parent().hide()
...
function ajax_save( on_success, is_wizard , callback ){
  if( is_wizard == 1 ){
    ...
    var ok_to_save = eval('validate_fg'+fg_id+'()')
  	...
  	var flag_error = $('#competency_master_id').val()

    if( $('label[for="is_core"]').parent().find('input[value="1"]').attr('checked') == 'checked' ){
     if (flag_error == ' ') {
        add_error('competency_master_id', 'Competency', "This field is mandatory.");
      };
  	}
