...
$(document).ready(function () { 
    ...
    $('.icon-16-rehire').live('click', function(){
        ...
        $.ajax({
            ...
            data: 'employee_id=' + $(this).parent().parent().parent().attr("id"),
            type: 'post',
            dataType: 'json',
            beforeSend: function(){
            ...
            success: function (response) {
                if(response.data.blacklisted == 1){
                    new Boxy('<p>Employee is Blacklisted and therefore cannot be hired.</p>',
                    {
                        title: 'Re-hire',
                        draggable: false,
                        modal: true,
                    ...
                    record_action("edit/rehire", $(elem).parent().parent().parent().attr("id"), $(elem).attr('module_link'));
                }
            }
        });
    });
    ...
    if(module.get_value("view") == "edit") {
        ...
        if($('#record_id').val() != -1)
        {
            ...
            {
                $('#record-form').append("<input type='hidden' name='previous_record_id' value='"+$('#record_id').val()+"'>");
                $('#record_id').val("-1");
                // $('#id_number').val("");
                // $('#biometric_id').val("");
            ...
            //tirso - to default first option company as selected
            $('#company_id').val($("#company_id option:eq(1)").val());
    ...
    $('#user_benefit_id').live('change', function() {
        if ($(this).val() != '') {
            $.ajax({
            url: module.get_value('base_url') + 'employees/get_benefits',
                type: 'post',
                dataType: 'json',
                data: 'user_benefit_id=' + $(this).val(),
                success: function(response) {
                    $('#benefits').val(response.user_benefit_description);
                    $('#benefits').text(response.user_benefit_description);
                }
            });
        } else {
            $('#benefits').val('');
   ...
   window.onload = function(){
        ...
        */
        ...
        if(module.get_value('view') == 'edit'){
            ...
            $('.project_name_id').live('change',function(){
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if ($(parent_elem).find(".work_assignment").val() == 2) {
                            $(parent_elem).find('.cost_code').val(response.cost_code);
                        };
                        $(parent_elem).find('.cost_code-project').val(response.cost_code);
                        $(parent_elem).find('.cost_code-division').val(response.div_code);
            ...
            $('.group_name_id').live('change',function(){
                var parent_elem = $(this).closest('.form-multiple-add');
                $.ajax({
                    url: module.get_value('base_url') + module.get_value('module_link') + '/get_group_cost_code',
                    data: 'group_name_id=' + $(this).val(),
                    type: 'post',
                    dataType: 'json',
                    beforeSend: function(){
                        //$('#appraisal-template-criteria-container').block({ message: '<div class="now-loading align-center"><img src="'+module.get_value('base_url')+user.get_value('user_theme')+'/images/loading.gif"><br />Loading, please wait...</div>' });          
                    ...
                    success: function (response) {
                        if ($(parent_elem).find(".work_assignment").val() == 3) {
                            $(parent_elem).find('.cost_code').val(response.cost_code);
                        };
                        $(parent_elem).find('.cost_code-group').val(response.cost_code);
                    }
                });     
            })      
            ...
            $('.department_id').live('change',function(){
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if ($(parent_elem).find(".work_assignment").val() == 4) {
                            $(parent_elem).find('.cost_code').val(response.cost_code);
                        };
                        $(parent_elem).find('.cost_code-department').val(response.cost_code);
                        $(parent_elem).find('.cost_code-group').val(response.grp_code);
            ...
            $('.division_id').live('change',function(){
                var parent_elem = $(this).closest('.form-multiple-add');
                $.ajax({
                    url: module.get_value('base_url') + module.get_value('module_link') + '/get_division_cost_code',
                    ...
                    success: function (response) {
                        // $(parent_elem).find('.group_name_id').val(response.group_name_id)
                        if ($(parent_elem).find(".work_assignment").val() == 1) {
                            $(parent_elem).find('.cost_code').val(response.cost_code);    
                        };
                        $(parent_elem).find('.cost_code-division').val(response.cost_code);  
                    }
                });     
            })
            ...
            if ($('#record_id').val() != '-1'){
                $.ajax({
                    url: module.get_value('base_url') + module.get_value('module_link') + '/get_employee',
                    data: 'employee_id=' + $('#record_id').val(),
                    type: 'post',
                    dataType: 'json',
                    beforeSend: function(){
                        //$('#appraisal-template-criteria-container').block({ message: '<div class="now-loading align-center"><img src="'+module.get_value('base_url')+user.get_value('user_theme')+'/images/loading.gif"><br />Loading, please wait...</div>' });          
                    ...
                    success: function (response) {
                        $('<input type="text" class="input-text" value="'+ response.data.aux +'" id="aux" name="aux" style="width:100px">').insertAfter('#lastname');
                        if(response.data.resigned == 0){
                            $('label[for="blacklisted"]').parent().hide();
                        }
                    }
                });
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    $('label[for="firstname"]').next().append('<span>'+ response.data.aux +'</span>');
                    if(response.data.resigned == 0){
                        $('label[for="blacklisted"]').parent().hide();
                    }
                }
...
/*        $('#employed_date-temp').live('change', function(){
            $('#leave_accumulation_start_date').val($(this).val());
            $('#leave_accumulation_start_date-temp').val($(this).val()); 
        });*/
...
/*                $('label[for="work_assignment[assignment]"]').parent().nextAll().hide();*/
                ...
                //display school and degree dropdown on add education
                 $('select[name="education[education_level][]"]').live('change', function () {
                    if ($(this).val() == '') {
                        return;
                    }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.type == 'error') {
                ...
                if (response.data.employed_date != null) {
                ...
                $.ajax({
                    ...
                    success: function(response)
                    {
                        if (response.msg_type != 'error'){
                            $('#id_number').val(response.employee_id_last_series);
                            $('#last_series_no').val(response.last_series);
                            // $('<input name="last_series_no" type="hidden" value="'+response.last_series+'">').insertAfter('#record_id');
                        }
                        else{
                            $('#id_number').val('');
                ...
                $('#lastname').width('260');
                if (response.data.aux != null) {
                    $('<input type="text" class="input-text" value="'+ response.data.aux +'" id="aux" name="aux" style="width:100px">').insertAfter('#lastname');
                }else{
                    $('<input type="text" class="input-text" value="" id="aux" name="aux" style="width:100px">').insertAfter('#lastname');
                };
               
                $('label[for="status_id"]').parent('div').hide();

                $('#employed_date-temp').addClass('popup');
                ...
                $('#birth_date-temp').live('change', function(){
                    var age = get_age(  $('#birth_date').val() );
                    $('span.calculatedage').html('&nbsp;and is now&nbsp;<u>&nbsp;'+age+"&nbsp;</u>&nbsp;years of age");
                });                
            }
    ...
    setTimeout(
        function () {
            ...
            if( module.get_value('view') == "detail" ){
                ...
                if ($('#employed_date-temp').hasClass('popup')){
                    return false;
                }
                else{
...
function show_work_assignment(assignment_type,parent_elem){
    ...
    if (assignment_type == 1){
        ...
        $(parent_elem).find('.cost_code-division').parent().parent().hide();
        $(parent_elem).find('.cost_code-project').parent().parent().hide();
        $(parent_elem).find('.cost_code-group').parent().parent().hide();
        $(parent_elem).find('.cost_code-department').parent().parent().hide();
        $(parent_elem).find('.project_name_id').parent().parent().removeClass('even').addClass('odd');
        ...
        // $(parent_elem).find('.project_name_id').parent().parent().removeClass('even').addClass('odd');
        // $(parent_elem).find('.department_id').parent().parent().removeClass('even').addClass('odd');
        $(parent_elem).find('.cost_code').parent().parent().hide();
    }   
}
...
function assignment_conc(show_hide,parent_elem){
    if (show_hide == 0){
        ...
        $(parent_elem).find('.cost_code-division').parent().parent().show();
        $(parent_elem).find('.project_name_id').parent().parent().show();
        $(parent_elem).find('.cost_code-project').parent().parent().show();
        $(parent_elem).find('.group_name_id').parent().parent().show();
        $(parent_elem).find('.cost_code-group').parent().parent().show();
        $(parent_elem).find('.department_id').parent().parent().show();
        $(parent_elem).find('.cost_code-department').parent().parent().show();
        $(parent_elem).find('.project_name_id').parent().parent().removeClass('even').addClass('odd');
        $(parent_elem).find('.department_id').parent().parent().removeClass('even').addClass('odd');
        $(parent_elem).find('.cost_code').parent().parent().hide();
...
function assignment_category_show(category_id,parent_elem){
    switch (category_id){
        ...
        case "4":
            ...
            $(parent_elem).find('.end_date').val('').parent().parent().show();                    
            $(parent_elem).find('.code_status_id').val('').parent().parent().show();
            // $(parent_elem).find('.department_id').val('').parent().parent();
            $(parent_elem).find('.code_status_id').val('').parent().parent().removeClass('odd').addClass('even');                 
            $(parent_elem).find('.cost_code').val('').parent().parent().show();  
            break;
        default:
...
function reset_elem_case(parent_elem){
    ...
    if ($(parent_elem).find('.division_id').val() != ''){
        $(parent_elem).find('.division_id').trigger('change');
        $(parent_elem).find('.division_id').parent().parent().show();
    ...
    if ($(parent_elem).find('.project_name_id').val() != ''){
        $(parent_elem).find('.project_name_id').trigger('change');
        $(parent_elem).find('.project_name_id').parent().parent().show();
        $(parent_elem).find('.code_status_id').parent().parent().show;
    }       
    else{
    ...
    if ($(parent_elem).find('.group_name_id').val() != ''){
        $(parent_elem).find('.group_name_id').trigger('change');
        $(parent_elem).find('.group_name_id').parent().parent().show();
        $(parent_elem).find('.code_status_id').parent().parent().hide;
    }   
    else{
    ...
    if ($(parent_elem).find('.department_id').val() != ''){
        $(parent_elem).find('.department_id').trigger('change');
        $(parent_elem).find('.department_id').parent().parent().show();
        $(parent_elem).find('.code_status_id').parent().parent().hide;
        $(parent_elem).find('.group_name_id').parent().parent().show();
...
function ajax_save_custom( on_success, is_wizard , callback ){
    if( is_wizard == 1 ){
