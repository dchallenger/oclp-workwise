        ...
        $.ajax({
            ...
            success: function (balance) {
                ...
                if(balance.show_carried == 1)
                {
                    ...
                    ( (parseFloat(balance.vl) - (parseFloat(balance.vl_used)+parseFloat(balance.el_used))).toFixed(2) == 'NaN' ) ? $('#vl_balance').empty() :  $('#vl_balance').empty().html((parseFloat(balance.vl) - ((parseFloat(balance.vl_used)+0.00)+(parseFloat(balance.el_used)+0.00))).toFixed(2)) ;
                ...
                if(balance.show_carried == 2)
                {
                    ...
                    var total_vl_used = parseFloat(balance.vl_used)+parseFloat(balance.el_used);
                    ( total_vl_balance == 0) ? $('#vl_balance').html('0.00') : $('#vl_balance').empty().html(parseFloat(total_vl_balance - total_vl_used).toFixed(2)) ;
         ...
         $.ajax({
            ...
            success: function (response) {     
                if(response.data != 1){
                    ...
                    $('#return_date-temp').replaceWith('<input type="textbox" id="return_date-temp" value="'+$('#return_date').val()+'" readonly=readonly />');
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (balance) {
                if(balance.show_carried == 1)
                {
                    ...
                    ( (parseFloat(balance.vl) - (parseFloat(balance.vl_used)+parseFloat(balance.el_used))).toFixed(2) == 'NaN' ) ? $('#vl_balance').empty() :  $('#vl_balance').empty().html(((parseFloat(balance.vl) + (parseFloat(balance.carried_vl))) - ((parseFloat(balance.vl_used)+0.00)+(parseFloat(balance.el_used)+0.00))).toFixed(2)) ;
...
function change_status_cancel(record_id, form_status_id, callback, cancel_remarks) {
    var data = 'record_id=' + record_id + '&form_status_id=' + form_status_id;

    if(cancel_remarks){
        data += '&cancel_remarks='+cancel_remarks;
    } else {
    ...
    $.ajax({
        url: module.get_value('base_url') + module.get_value('module_link') + '/change_status_cancel',
        data: data,
        type: 'post',
        dataType: 'json',
        success: function(response) {
