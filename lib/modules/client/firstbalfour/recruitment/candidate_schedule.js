$(document).ready(function(){
	...
	window.onload = function(){
		...
		if( module.get_value('view') == "edit" ){
			$('select[name="interviewer_id[]"]').chosen();
	...
	if (module.get_value('view') == 'edit'){
		...
		$('#current_candidate_status').val($('#candidate_status_id option:selected').text());
...
/*		$('#mrf_id').attr('disabled',true);	
	...
	$('#save_applicant').live('click',function(){
 	...
 	$("#candidate_status_id").change(function() {
 		// console.log()
 		var selected_status = $("#candidate_status_id").val();
	...
	$('.icon-send').live('click',function(){
		var applicant_id = $('#applicant_id').val();
		var applicant_employee_id = $('#employee_id').val();
		
		if ($(this).hasClass('technical')){
			var employee_id = $(this).closest('div.parent_container').find('.final_interviewer_id').val();
			var date_time = $(this).closest('div.parent_container').find('input[name="final_interview_date[]"]').val();
		}
		else{
...
function ajax_save_candidate( on_success, is_wizard , callback ){
	if( is_wizard == 1 ){
		...
		var selected_status = $("#candidate_status_id").val();
 		if (selected_status == '3') {
	 		var employee_id = $('.icon-send').closest('div.parent_container').find('.interviewer_id').val();
			var date_time = $('.icon-send').closest('div.parent_container').find('input[name="interview_date[]"]').val();
...
function add_technical_interviewer(){
	...
	$.ajax({
		...
		success: function ( response ) {
			...
			var ctrval = parseFloat($('#ctr_handler').val());
			var cls = "datetimepicker"+ctrval+"";
			$('#technical-interviewer-container').append(response).find('input').addClass(cls);
			$('#ctr_handler').val(ctrval + 1);
...
function delete_benefit( field ){
	...
	function( choice ) {
		if(choice == "Yes"){
			...
			var interviewer_id = field.parent().parent().parent().find('.interviewer_id').val();
			
			if (interviewer_id == undefined) {
				interviewer_id = field.parent().parent().parent().find('.final_interviewer_id').val();
			};
			
			var candidate_id = $("#record_id").val();
			var data = 'interviewer='+interviewer_id+"&candidate="+candidate_id; 

			simpleAjax( module.get_value('base_url') + module.get_value('module_link') + '/delete_interviewer', data );
