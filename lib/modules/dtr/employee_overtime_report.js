...
var colModel = [
    ...
    {name: 'ot_hours', sortable: false, width: 50, align: 'center'},
    {name: 'ot_code', sortable: false, width: 100, align: 'center'},
...
{	 
    ...
    if(date_from != '' && date_to != '') {
        $('#overtime_export').attr('action', $('#export_link').val());
        $('#overtime_export').submit();
    } else {
...
function get_data(){
    ...
    var date_from = $('input[name="date_from"]').val();
    var date_to = $('input[name="date_to"]').val();
    var colNames = ["ID", "CC", "OT Hrs", "OT Code", "OT Amt"];
