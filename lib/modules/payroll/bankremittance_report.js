$( document ).ready( function() 
{
    $.ajax({
        ...
        success: function ( response ) 
        {
            ...
            paycode = paycode + '<label class="label-desc gray" for="paycode_id"> Paycode: <span class="red">*</span></label>';
            paycode = paycode + '<div class="multiselect-input-wrap">';
            paycode = paycode + response.paycode_html;
            paycode = paycode + '</div>';
            $('label[for="tran_type"]').parent().after(paycode);
...
function generate_report()
{
    if($('select[name="company_id"]').val() != '')
    {
        ...
        {
            //$('#`-form input[name="print_type"]').val('1');
