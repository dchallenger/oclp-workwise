$( document ).ready( function() 
{
    $.ajax({
        ...
        success: function ( response ) 
        {
            ...
            $('label[for="cost_code_id"]').parent().remove();
            var cost_code = '<div class="form-item odd ">';
            cost_code = cost_code + '<label class="label-desc gray" for="cost_code_id"> Cost Code: </label>';
            cost_code = cost_code + '<div class="multiselect-input-wrap">';
            cost_code = cost_code + response.cost_code_html;
            cost_code = cost_code + '</div>';
            $('label[for="company_id"]').parent().after(cost_code);
            $('#cost_code_id').multiselect().multiselect({
                show:['blind',250],
                hide:['blind',250],
                selectedList: 1
            });
