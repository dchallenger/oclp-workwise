...
{    
    $.ajax({
        ...
        {   
            ...
            $('label[for="code_status_id"]').parent().remove();
            var code_status = '<div class="form-item even ">';
            code_status = code_status + '<label class="label-desc gray" for="code_status_id"> Code Status: </label>';
            code_status = code_status + '<div class="multiselect-input-wrap">';
            code_status = code_status + response.code_status_html
            code_status = code_status + '</div>';
            $('label[for="paycode_id"]').parent().before(code_status);
            $('#code_status_id').multiselect().multiselectfilter({
                show:['blind',250],
                hide:['blind',250],
                selectedList: 1
            }); 
            ...
            paycode = paycode + '</div>';
            $('label[for="payroll_date"]').parent().before(paycode);
