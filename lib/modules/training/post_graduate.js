$(document).ready(function() {
    ...
    if (status == 3) {
        ...
        var training_type = 3 //$('#training_type').val();
        if (module.get_value('view') == 'edit') {
            ...
            var rank_id = $('#rank_id').val();
            var employee_id = $('#employee_id').val();
            get_budget(rank_id, employee_id, date_from);
        };

        if ($('#stb').val() != "" && parseFloat($('#stb').val()) > 0) {
    ...
    $('#date-temp-from').attr('disabled', false).attr('readonly', true);
	$('#date-temp-from').live('change', function() {
        var rank_id = $('#rank_id').val();
        var employee_id = $('#employee_id').val();
        var date_from = $('#date_from').val();
        get_budget(rank_id, employee_id, date_from);
...
function get_employee_details(employee_id) {
	$.ajax({
        ...
        success: function(data){
            ...
            if (data.tenure < 5 ) {
                $('input[name="tenure"]').attr('disabled', true);
            }else{
                $('input[name="tenure"]').attr('disabled', false);
                // $('input[name="tenure"]').attr('checked', 'checked');
            };

            $.unblockUI();
            if (parseInt(data.rank) < 7 || data.tenure < 5 ) {
                // $('#message-container').html(message_growl('error', 'You are not allowed to apply yet'));
                setTimeout(function () {
                    go_to_previous_page( 'You are not allowed to apply yet.' );
                }, 3000);
            }else{
                $('input[name="tenure"]').attr('checked', 'checked');
                $('input[name="level_seven"]').attr('checked', 'checked');
            };
        }           
    });
}
...
function get_budget(rank_id, employee_id, date_from) {
	$.ajax({
        url: module.get_value('base_url') + module.get_value('module_link') + '/get_budget',
        data: 'rank_id=' + rank_id + '&employee_id=' + employee_id + '&date=' + date_from + '&record_id='+module.get_value('record_id'),
        type: 'post',
        dataType: 'json',
        beforeSend: function(){
            $.blockUI({
...
function validate_fg689() {
	validate_mandatory_array('action[plan][]', "Action Plan");
	return process_errors();
}

function validate_fg690() {
	validate_mandatory('transfer[transfer][]', "Knowledge Transfer");
	return process_errors();
}
...
function ajax_save( on_success, is_wizard , callback ){
    ...
    if( ok_to_save ) { 
        $('#department_id, #division_id, #position_id, #rank_id, #stb, #investment').attr('disabled', false);     
