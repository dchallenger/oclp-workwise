$(document).ready(function () {
    ...
    $('a.search_participants').live('click', function () {
        window.location = module.get_value('base_url') + 'training/training_feedback_participants/index/' + $(this).parents('tr').attr('id');
    });

    $('a.view_feedback').live('click', function () {

        $('#record-form').append('<input id="participant_direct" type="hidden" value="1" name="participant_direct" />');

        var url = module.get_value('base_url') + module.get_value('module_link') + '/get_feedback_participant_id';
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if( response.feedback_id != 0 ){
                    record_action("detail", response.feedback_id, module_link);
                }
            }
    ...
    $('a.edit_feedback').live('click', function () {

        $('#record-form').append('<input id="participant_direct" type="hidden" value="1" name="participant_direct" />');

        window.location = module.get_value('base_url') + 'training/training_feedback_participants/edit/' + $(this).parents('tr').attr('id');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if( response.feedback_id != 0 ){
                    record_action("edit", response.feedback_id, module_link);
                }
            }
