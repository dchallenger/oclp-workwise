...
if ( !empty($_FILES) ) {	
	...
	$name = sanitize_file_name($_FILES['Filedata']['name']);
	$path = $_POST['fullpath'] . '/'. $_POST['path'] .'/' . $date . '_' . $name;
	...
	echo $_POST['path'] .'/' . $date . '_' . str_replace(' ', '_', $_FILES['Filedata']['name']).'|||||'.$_POST['text_id'].'|||||'.$response->file_type;
}
